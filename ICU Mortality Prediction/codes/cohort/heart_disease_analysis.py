...
MIMIC_DIR = '/Users/FJG1030/mimiciii/mimic-data'
...
Diagnoses_ICD = pd.read_csv(MIMIC_DIR+'/DIAGNOSES_ICD.csv')
...
print('Unique Visits:', len(unique_visits))
...
# Find Age at Death and remove NANs
...
Admissions_Time = (Release_Series - Admit_Series)  # time [days] that the patients stayed in admission
Admit_Time = []
...
Patients['DOB-2'] = Birth_Date_Series  # New column with usable DOB
Patients['DOD-2'] = Death_Series  # New column with usable DOD
Patients['AOD'] = Age_Death  # New column with usable AOD
Admissions['Admit'] = Admit_Series  # New column with usable Admit date
Admissions['Release'] = Release_Series  # New column with usable Release date
Admissions['Total Admission Time'] = Admissions_Time  # New column with Total Admission time

Admissions_culled = Admissions.drop_duplicates('SUBJECT_ID', keep='last')
Admissions_culled_Admits = Admissions_culled['ADMITTIME']

Admit_Ages = (Admit_Series - Birth_Date_Series)  # Age at which the patients were admitted

# Keep only last admission
...
# Remove nulls from admissions and usable admissions date
Atimes = []
for n in range(len(Admit_times2)):
    if pd.isnull(Admit_times2[n]):
...
for n in range(len(DOB_Admit_times)):
    if pd.isnull(DOB_Admit_times[n]):
...
cdeath = []  # cause of death (diagnosis)
subjectid = []  # subject unique
ldeath = []  # location of death
ideath = []  # insurance type
edeath = []  # ethnicity
tdeath = []  # admission type
hamdiddeath = []  # hospital admissions id
admitt = []  # total admissions time
...
heart_death_desc = Death.loc[Death['CAUSE'].str.contains("HEART|MYOCARDIAL|CARDIAC", na=False) == True]
heart_attack = Death.loc[Death['CAUSE'].str.contains("MYOCARDIAL", na=False) == True]
...
death_flag = []
for entry in range(len(demographics)):
    if not pd.isnull(demographics['DOD'][entry]):
        death_flag.append(1)
    else:
        death_flag.append(0)
...
for entry in range(len(demographics)):
    if pd.isnull(demographics['HEART_ATTACK_FLAG'][entry]):
...
for entry in range(len(demographics)):
    if pd.isnull(demographics['CAUSE_y'][entry]):
